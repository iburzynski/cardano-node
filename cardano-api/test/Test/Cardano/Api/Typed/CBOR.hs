...
prop_roundtrip_Tx_Cddl = H.property $ do
  AnyCardanoEra era <- H.forAll $ Gen.element [minBound..AnyCardanoEra AlonzoEra] --TODO: Babbage era
  x <- forAll $ genTx era
  H.tripping x serialiseTxLedgerCddl (deserialiseTxLedgerCddl era)
...
prop_roundtrip_TxWitness_Cddl = H.property $ do
  ...
  x <- forAll $ genShelleyKeyWitness $ shelleyBasedToCardanoEra era
  tripping x (serialiseWitnessLedgerCddl era) (deserialiseWitnessLedgerCddl era)
